...
public class EventService {
    ...
    public List<Event> getAllEvents() {
        return EventEntity.<EventEntity>listAll().stream()
                .map(this::toModel)
                .toList();
    }
    ...
    public Event createEvent(CreateEventRequest request) {
        ...
        entity.type = EventEntity.EventType.fromString(request.getType().value());
    ...
    public Optional<Participant> addParticipant(String eventId, AddParticipantRequest request) {
        ...
        boolean alreadyRegistered = entity.participants.stream()
                .anyMatch(p -> p.cyclistId.equals(request.getCyclistId()));
    ...
    public List<LeaderboardEntry> getLeaderboard(String eventId) {
        ...
        List<PositionEntity> latestPositions = PositionEntity.find(
            "eventId = ?1", eventId
        ).list();
        
        // Group by cyclist and get latest position for each
        var positionsByCyclist = latestPositions.stream()
        ...
        var entries = positionsByCyclist.entrySet().stream()
    ...
    public List<Position> getCurrentPositions(String eventId) {
        List<PositionEntity> positions = PositionEntity.find(
            "eventId = ?1 ORDER BY timestamp DESC", eventId
        ).list();
        
        // Get latest position for each cyclist
        var latestPositions = positions.stream()
    ...
    private Event toModel(EventEntity entity) {
        ...
        event.setParticipants(entity.participants.stream()
                .map(this::toModelParticipant)
                .toList());
    ...
    private Route toModelRoute(EventEntity.Route entityRoute) {
        ...
        if (entityRoute.waypoints != null) {
            route.setWaypoints(entityRoute.waypoints.stream()
                    .map(this::toModelWaypoint)
                    .toList());
        }
        return route;
    }
    ...
    private Waypoint toModelWaypoint(EventEntity.Waypoint entityWaypoint) {
        ...
        waypoint.setElevation(entityWaypoint.elevation != null ? BigDecimal.valueOf(entityWaypoint.elevation) : null);
        return waypoint;
    }
    ...
    private Position toModelPosition(PositionEntity entity) {
        ...
        position.setLocation(toModelGeoPoint(new EventEntity.GeoPoint(
    ...
    private EventEntity.Route toEntityRoute(Route modelRoute) {
        ...
        if (modelRoute.getWaypoints() != null) {
            route.waypoints = modelRoute.getWaypoints().stream()
                    .map(this::toEntityWaypoint)
                    .toList();
        }
        return route;
    }
    ...
    private EventEntity.GeoPoint toEntityGeoPoint(GeoPoint modelPoint) {
        return new EventEntity.GeoPoint(modelPoint.getLatitude().doubleValue(), modelPoint.getLongitude().doubleValue());
    }
