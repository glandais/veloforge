...
public class SimulationService {
  ...
  private void updateCyclistPosition(
      String eventId, EventEntity.Participant participant, EventEntity event) {
  ...
  private PositionEntity createInitialPosition(
      String eventId, String cyclistId, EventEntity.Route route) {
    PositionEntity position = new PositionEntity();
    position.eventId = eventId;
    position.cyclistId = cyclistId;
    position.location =
        new PositionEntity.GeoPoint(route.startPoint.latitude, route.startPoint.longitude);
  ...
  private PositionEntity calculateNextPosition(
      PositionEntity lastPosition, CyclistEntity cyclist, EventEntity.Route route) {
    ...
    double effectivePower =
        calculateEffectivePower(cyclist, lastPosition.power != null ? lastPosition.power : 250.0);
  ...
  private void updateCyclistFatigue(CyclistEntity cyclist, double speed) {
    ...
    cyclist.state.muscularFatigue =
        Math.min(100.0, cyclist.state.muscularFatigue + fatigueIncrease);
